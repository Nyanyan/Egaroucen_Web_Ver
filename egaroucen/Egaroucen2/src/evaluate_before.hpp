...
inline bool init_evaluation_calc(){
    ...
    */
    constexpr double all_params[] = {PARAM_REPLACE_HERE};
        ...
        for (pattern_idx = 0; pattern_idx < N_PATTERNS; ++pattern_idx){
            for (i = 0; i < N_DENSE0; ++i){
                for (j = 0; j < pattern_sizes[pattern_idx] * 2; ++j){
                    dense0[i][j] = all_params[all_param_idx++];
                }
            }
            for (i = 0; i < N_DENSE0; ++i){
                bias0[i] = all_params[all_param_idx++];
            }
            for (i = 0; i < N_DENSE1; ++i){
                for (j = 0; j < N_DENSE0; ++j){
                    dense1[i][j] = all_params[all_param_idx++];
                }
            }
            for (i = 0; i < N_DENSE1; ++i){
                bias1[i] = all_params[all_param_idx++];
            }
            for (i = 0; i < N_DENSE1; ++i){
                dense2[i] = all_params[all_param_idx++];
            }
            bias2 = all_params[all_param_idx++];
